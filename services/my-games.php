...
$sql = "SELECT ContactID, PlayerEmail, fieldID, RatingReceived FROM PlayerFieldContact WHERE PlayerEmail = ?";
...
<head>
    ...
    <title>Your Games and Ratings</title>
    <style type="text/css">
        table {
            margin: 8px;
...
<body>
    ...
    <center>
        <h1>Welcome to your games and rating!</h1>
    </center>
    <?php
    if ($result->num_rows > 0) {
        echo "<center> <table><tr><th>ID</th><th>Player Email</th><th>Field ID</th><th>Rating</th></tr>";
        while ($row = $result->fetch_assoc()) {
            echo "<tr><td>" . htmlspecialchars($row["ContactID"]) . "</td><td>" . htmlspecialchars($row["PlayerEmail"]) . "</td><td>" . htmlspecialchars($row["fieldID"]) . "</td><td>" . htmlspecialchars($row["RatingReceived"]) . "</td></tr>";
        }
        echo "</table></center>";
    } else {
